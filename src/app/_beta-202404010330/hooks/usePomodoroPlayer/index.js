...
const usePomodoroPlayer = () => {
  ...
  const _playNext = () => {
    ...
    console.log('checking if to play next ', playerState.currentTrackObj.value.played, playerState.currentTrackObj.value.repeat)
    ...
    if(parentTrackPosition.length === 0) {
      console.log('loggr skip to next ')
      skipToNext();
      return;
    }
  ...
  const skipToNext = () => {
    ...
    const nextTrackIndex = findNextSkipTrackIndex();
    // console.log('loggr skip to next track', nextTrackIndex)
...
export default usePomodoroPlayer;